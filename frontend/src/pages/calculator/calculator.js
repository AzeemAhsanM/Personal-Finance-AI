...
const Calculator = () => {
  ...
  useEffect(() => {
    ...
    const n = sipYears * 12;        // months

    let M = 0;
    if (i === 0) {
      M = sipAmount * n; // no interest
    } else {
      M = sipAmount * ((Math.pow(1 + i, n) - 1) / i) * (1 + i);
    }
  ...
  useEffect(() => {
    const P = emiAmount;
    const r = (emiRate / 100) / 12; // monthly rate
    const n = emiMonths;
      ...
      <div className="calculator-card">
        ...
        <div className="calculator-toggle">
          ...
          <button
            ...
            className={activeCalculator === 'emi' ? 'active' : ''}
          >
            EMI Calculator
          </button>
        </div>
        ...
        {activeCalculator === 'sip' && (
          <div className="calculator-content">
            <div className="form-group-calc">
              <label>
                Monthly Investment: ₹{Number(sipAmount).toLocaleString('en-IN')}
              </label>
              <input
                type="range"
                min="100"
            ...
            <div className="form-group-calc">
              <label>
                Expected Return Rate (% p.a.): {sipRate}%
              </label>
              <input
                type="range"
                min="1"
            ...
            <div className="result-section">
              <p>
                Invested Amount: <span>₹{Number(sipResult.invested).toLocaleString('en-IN')}</span>
              </p>
              <p>
                Est. Returns: <span>₹{Number(sipResult.returns).toLocaleString('en-IN')}</span>
              </p>
              <p className="total-value">
                Total Value: <span>₹{Number(sipResult.total).toLocaleString('en-IN')}</span>
              </p>
            </div>
          </div>
        )}
        ...
        {activeCalculator === 'emi' && (
          <div className="calculator-content">
            <div className="form-group-calc">
              <label>
                Loan Amount: ₹{Number(emiAmount).toLocaleString('en-IN')}
              </label>
              <input
                type="range"
                min="10000"
            ...
            <div className="result-section">
              <p>
                Principal Amount: <span>₹{Number(emiResult.principal).toLocaleString('en-IN')}</span>
              </p>
              <p>
                Total Interest: <span>₹{Number(emiResult.interest).toLocaleString('en-IN')}</span>
              </p>
              <p>
                Total Payable: <span>₹{Number(emiResult.total).toLocaleString('en-IN')}</span>
              </p>
              <p className="total-value">
                Monthly EMI: <span>₹{Number(emiResult.monthlyEmi).toLocaleString('en-IN')}</span>
              </p>
            </div>
          </div>
        )}
